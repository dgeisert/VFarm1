...
public class Input_Inventory : InputMachine {
	...
	public override void InstanceUpdate(StateMachine checkMachine){
		if (!checkMachine.GetComponent<InputMachine>().is_holding) {
			...
			checkMachine.GetComponent<InputMachine> ().recticle.right.SetHand ("Invetory");
		}
	}
	...
	public override void Hold(GameObject obj, Vector3 point, StateMachine checkMachine){
		if (obj == null) {
			...
			checkMachine.GetComponent<InputMachine> ().recticle.right.SetHand ("Inventory");
			return;
		}
		...
		checkMachine.GetComponent<InputMachine> ().recticle.SetReticle ("Inventory");
		checkMachine.GetComponent<InputMachine> ().recticle.right.SetHand ("none");
	}
